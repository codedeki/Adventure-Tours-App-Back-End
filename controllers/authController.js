...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    httpOnly: true, // can't modify in browser (csrf attacks)
  };
  //only send by https if in production
  if (process.env.NODE_ENV === 'production') cookieOptions.secure = true;

  res.cookie('jwt', token, cookieOptions);

  // Remove pasword from output
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Set new password if token expired
  if (!user) {
    return next(new AppError('Token is invalid or has expired', 400));
  }
  // 3) update password
  user.password = req.body.password; //store password in variable
  user.passwordConfirm = req.body.passwordConfirm;
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  await user.save(); //validate
